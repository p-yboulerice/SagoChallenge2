namespace SagoAnimationEditor {
	...
	public class CompositeAnimationWindow : EditorWindow {
		
		
		#region Menu Methods
		
		[MenuItem("Window/Sago/Composite Animation")]
		public static void GetWindow() {
			EditorWindow.GetWindow<CompositeAnimationWindow>();
		}
		...
		private static string AnimationKey = "SagoAnimationEditor.Window.CompositeAnimationWindow.Animation";
		private static string FrameIndexKey = "SagoAnimationEditor.Window.CompositeAnimationWindow.FrameIndex";
		private static string IsLoopKey = "SagoAnimationEditor.Window.CompositeAnimationWindow.IsLoop";
		...
		private AudioAnimation AudioAnimation {
			get { return Animation ? Animation.AudioAnimation : null; }
		}
		...
		private MeshAnimation MeshAnimation {
			get { return Animation ? Animation.MeshAnimation : null; }
		}
		...
		private void SyncFrameCount() {
			if (AudioAnimation && MeshAnimation) {
				AudioAnimation.FrameCount = MeshAnimation.Frames.Length;
			}
		}
		
		private void SyncFrameRate() {
			if (AudioAnimation && MeshAnimation) {
				AudioAnimation.FramesPerSecond = MeshAnimation.FramesPerSecond;
			}
		}
		...
		private void OnEnable() {
			...
			Animation = AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(EditorPrefs.GetString(AnimationKey)), typeof(CompositeAnimation)) as CompositeAnimation;
			...
			AudioAnimationView.FrameCountSyncedImpl = () => {
				return (
					!AudioAnimation ||
					!MeshAnimation || 
					AudioAnimation.FrameCount == MeshAnimation.Frames.Length
			...
			AudioAnimationView.FrameRateSyncedImpl = () => {
				return (
					!AudioAnimation ||
					!MeshAnimation || 
					AudioAnimation.FramesPerSecond == MeshAnimation.FramesPerSecond
			...
			AudioAnimationView.SyncFrameCountImpl = SyncFrameCount;
			AudioAnimationView.SyncFrameRateImpl = SyncFrameRate;
			...
			ScrollPosition = EditorGUILayout.BeginScrollView(ScrollPosition, GUILayout.Width(position.width), GUILayout.Height(position.height));
			{
				...
				EditorGUILayout.BeginVertical(sectionStyle, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(false));
				{
					...
					GUILayout.BeginHorizontal(GUIStyle.none);
					{
						...
						Animation = EditorGUILayout.ObjectField("Animation", Animation, typeof(CompositeAnimation), false) as CompositeAnimation;
						if (EditorGUI.EndChangeCheck()) {
							Timer.GoToAndStop(0);
							RepaintLater();
						}
						if (GUILayout.Button("New", buttonStyle, GUILayout.ExpandWidth(false))) {
							Animation = CompositeAnimationEditor.CreateCompositeAnimation();
						}
					}
					...
					{
						AudioAnimation audioAnimation = Animation ? Animation.AudioAnimation : null;
						EditorGUI.BeginDisabledGroup(!Animation);
						{
						...
						if (Animation && Animation.AudioAnimation != audioAnimation) {
					...
					GUILayout.BeginHorizontal();
					{
						MeshAnimation meshAnimation = Animation ? Animation.MeshAnimation : null;
						EditorGUI.BeginDisabledGroup(!Animation);
						{
							meshAnimation = EditorGUILayout.ObjectField("Mesh Animation", meshAnimation, typeof(MeshAnimation), false) as MeshAnimation;
							EditorGUI.BeginDisabledGroup(true);
							{
								GUILayout.Button("New", buttonStyle, GUILayout.ExpandWidth(false));
							}
							EditorGUI.EndDisabledGroup();
						}
						EditorGUI.EndDisabledGroup();
						if (Animation && Animation.MeshAnimation != meshAnimation) {
				...
				{
					EditorPrefs.SetString(AnimationKey, AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(Animation)));
					Timer.FrameCount = FrameCount;
					Timer.FrameRate = FrameRate;
				}
				
				// timer view
				if (AudioAnimation != null || MeshAnimation != null) {
					EditorGUILayout.BeginVertical(sectionStyle, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(false));
						TimerView.OnGUI();
	...
	class AudioAnimationView : ScriptableObject {
				...
				if (Animation != null) {
					foreach (AudioAnimationElement element in Animation.Elements) {
						...
						if (element.AudioClip && Event.current.type == EventType.Repaint) {
							Texture2D texture = GetTexture(element.AudioClip, 0, (int)elementRect.width, (int)elementRect.height);
							if (texture) EditorGUI.DrawTextureTransparent(elementRect, texture);
		...
		private Texture2D GetTexture(AudioClip clip , int channel , int width , int height) {
			
			// @see: http://forum.unity3d.com/threads/reflected-audioutil-class-for-making-audio-based-editor-extensions.308133/
			
			Texture2D texture = null;
			
			// if (clip != null && width > 1 && height > 1) {
			//
			// 	Assembly unityEditorAssembly = typeof(AudioImporter).Assembly;
			// 	System.Type audioUtilClass = unityEditorAssembly.GetType("UnityEditor.AudioUtil");
			// 	MethodInfo method = audioUtilClass.GetMethod("GetWaveForm",	BindingFlags.Static | BindingFlags.Public);
			//
			// 	texture = (Texture2D)method.Invoke(null,new object[] {
			// 		clip,
			// 		AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(clip)) as AudioImporter,
			// 		channel,
			// 		width,
			// 		height
			// 	});
